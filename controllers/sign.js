...
exports.signup = function (req, res, next) {
  ...
  // 验证信息的正确性
  if ([name, email, university, phone, major, edu, gradate].some(function (item) { return item === ''; })) {
  ...
  // User.newAndSave(university,name, phone, email, major, edu ,gradate, function (err) {
  //   if (err) {
  //     return next(err);
  //   }
  //   res.render('sign/signup', {
  ...
  User.getUserByWeixinOpenId(req.session.user.weixin_openid, function (err, user) {
    if (err) {
      return next(err);
    }
